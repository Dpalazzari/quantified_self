...
function checkBoxFood(){
  var checkedFoods = $('tr input:checked');
  checkedFoods.each(function(element){
    return $('tr td#food-name').val();
  });
}

function checkBoxCalories(){
  var checkedCalories = $('tr input:checked');
  checkedCalories.each(function(element){
    return $('tr td#food-calories').val();
  });
}

function addCheckedFoodToMeal(checkBoxFood, checkBoxCalories){
  $('button#add-breakfast').on('click', function(){
    $('#breakfast-table').prepend("<tr id='food-row'><td class='selected'><input type='checkbox'></td><td class='food-name'>" + checkBoxFood + 
    "</td><td class='food-calories'>" + checkBoxCalories + "</tr>")
  });
  $('button#add-lunch').on('click', function(){
    $('#lunch-table').prepend("<tr id='food-row'><td class='selected'><input type='checkbox'></td><td class='food-name'>" + checkBoxFood + 
    "</td><td class='food-calories'>" + checkBoxCalories + "</tr>")
  });
  $('button#add-dinner').on('click', function(){
    $('#dinner-table').prepend("<tr id='food-row'><td class='selected'><input type='checkbox'></td><td class='food-name'>" + checkBoxFood + 
    "</td><td class='food-calories'>" + checkBoxCalories + "</tr>")
  });
  $('button#add-snacks').on('click', function(){
    $('#snacks-table').prepend("<tr id='food-row'><td class='selected'><input type='checkbox'></td><td class='food-name'>" + checkBoxFood + 
    "</td><td class='food-calories'>" + checkBoxCalories + "</tr>")
  });
}