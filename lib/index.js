...
function addFoodToMealTable(){
  ...
  prependCheckedFood('button#add-dinner', '#dinner-table', 200);
  prependCheckedFood('button#add-snacks', '#snack-table', 800);
}
...
function getDay() {
  $('.current-day section').html(currentDate.day(1).format("dddd, MM Do YYYY"));

  $('.current-day section').on('click', function() {
    $('.current-day section').html(currentDate.day(1).format("dddd, MM Do YYYY"));
  });
  $('button#forward').on('click', function() {
    $('.current-day section').html(currentDate.day(2).format("dddd, MM Do YYYY"));
  });
  $('button#back').on('click', function() {
    $('.current-day section').html(currentDate.day(0).format("dddd, MM Do YYYY"));
  });
};
...
function calculateRemainingCalories(){
  ...
  if(remainingCalories < 0){
    finalRemainingCalories.removeClass('positive').addClass('negative')
...
function calculateMealCalories(table, calories){
  ...
  if(finalTotal >= 0){
    ...
    remainingCalories.removeClass('positive').addClass('negative')
  }
}
...
function prependCheckedFood(button, mealTable, totalCalories){
  $(button).on('click', function(){
    ...
    $.each(rows, function(){
      ...
      appendFood(mealTable, food, calories)
    });
    var table = mealTable + '-header'
    var specificTable = table.replace('#', '.')
