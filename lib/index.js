...
function prependStorageFood(breakf, lunch, snack, dinner){
  if(breakf != null){
    breakf.forEach(function(element){
      foodStorage(element.name, element.calories, '#breakfast-table')
    })
    updateTotals();
  }
  if(lunch != null){
    lunch.forEach(function(element){
      foodStorage(element.name, element.calories, '#lunch-table')
    })
    updateTotals();
  }
  if(snack != null){
    snack.forEach(function(element){
      foodStorage(element.name, element.calories, '#snack-table')
    })
    updateTotals();
  }
  if(dinner != null){
    dinner.forEach(function(element){
      foodStorage(element.name, element.calories, '#dinner-table')
    })
    updateTotals();
  }
}
